...
class TextLoader():
    def __init__(self, data_dir, split, batch_size, seq_length, encoding='utf-8'):
    ...
    def create_batches(self):                                                   
        ...
        if self.num_batches == 0:
            assert False, "num_batches==0. Not enough data. Make seq_length and batch_size small."
        ...
        assert all(self.x_batches[-1][:,0]==self.vocab['^'])
        assert all(self.x_batches[-1][:,-1]==self.vocab['$'])
        assert all(self.y_batches[-1][:,-1]==self.vocab['^'])
        assert all(self.y_batches[-1][:,-2]==self.vocab['$'])
