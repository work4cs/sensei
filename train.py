...
parser.add_argument('--info_dir', type=str, default='info',
                    help='directory to store info pickle file')
...
parser.add_argument('--valid_epoch', type=int, default=10,
                    help='evey n epochs to validation')
...
def train(args):
    data_loader = TextLoader(args.data_dir, args.split, args.batch_size, args.seq_length)
    ...
    if args.init_from is not None:
        ...
        need_be_same = ["model", "rnn_size","emb_size", "num_layers", "seq_length"]
    ...
    with tf.Session() as sess:
        ...
        saver = tf.train.Saver(tf.global_variables(), max_to_keep=args.valid_epoch*2)
        # restore model
        if args.init_from is not None:
        ...
        for e in range(args.num_epochs):
            train_loss_total = 0
            #assign decay learning rate for each epoch
            ...
            for b in range(data_loader.num_batches):
                ...
                train_loss, state, _ = sess.run([model.cost, model.final_state, model.model_op], feed)
                train_loss_total += train_loss

            #save
            checkpoint_path = os.path.join(args.save_dir, 'model'+str(e)+'.ckpt')
            saver.save(sess, checkpoint_path) #, global_step=e
            checkpoint_path_list.append(checkpoint_path)
            
            if e%10 == 0:
              print("epoch {}, train_loss = {:.3f}".format(e,train_loss_total/data_loader.num_batches))
            
        print("epoch {}, train_loss = {:.3f}".format(e,train_loss_total/data_loader.num_batches))
            # print('Learning rate: %f' % (sess.run(model.lr)))
        info = pickle.load(open( args.info_dir, 'rb' ))
        info['epoch'] = e
        info['train_loss'] = train_loss_total/data_loader.num_batches
        pickle.dump(info, open( args.info_dir, 'wb' ))
              
                        
if __name__ == '__main__':
    train(args)
    print(str(args.rnn_size)+'-'+str(args.emb_size))
    print("!!!!!!!!!!!!!!finished")
